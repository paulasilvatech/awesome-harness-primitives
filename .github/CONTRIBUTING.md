...
python3 harness/github-copilot/scripts/validate_primitives.py --kind agents
python3 harness/github-copilot/scripts/validate_primitives.py --kind instructions
python3 harness/github-copilot/scripts/validate_primitives.py --kind skills
python3 harness/github-copilot/scripts/validate_primitives.py --kind prompts
python3 harness/github-copilot/scripts/validate_primitives.py --kind plugins
...
- Declare `.github/prompts/` publication in `harness/github-copilot/manifests/installed-primitives.json` only when workspace discovery is required.
- PR-gating rule IDs: `PR001` through `PR008`.

## Plugins

- Add each plugin under `harness/github-copilot/plugins/<name>/plugin.json` unless the spec-supported manifest location is needed.
...
- **The shared library is the source of truth.** Files under a plugin that also exist in
  `harness/github-copilot/{agents,instructions,skills}` are *generated copies* — plugins must be self-contained
  because upward-relative references (`../../agents/...`) do not resolve at install time. Edit the
  library original, then run `python3 harness/github-copilot/scripts/sync_plugin_components.py` to regenerate. Never
  hand-edit a plugin copy: `--check` runs in CI and fails the build on drift.
- PR-gating rule IDs: `PL001`, `PL002`, `PL003`, `PL008`.

## Hooks

- Add each hook package under `harness/github-copilot/hooks/<name>/hooks.json`; install copies as `.github/hooks/*.json` or `~/.copilot/hooks/*.json`.
