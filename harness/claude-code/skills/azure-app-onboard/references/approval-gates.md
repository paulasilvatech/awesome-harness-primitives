...
If "Run manually" is selected → point to [deploy-checklist-template.md § Deployment Summary](deploy/deploy-checklist-template.md) for manual execution steps.
...
> ⛔ **Before entering deploy:** ⛔ Read [`deploy/SKILL.md`](phases/deploy.md) before any deployment action. After mid-session compaction, re-read `deploy/SKILL.md` Steps 4-8. You MUST write `deploy-result.json`.
