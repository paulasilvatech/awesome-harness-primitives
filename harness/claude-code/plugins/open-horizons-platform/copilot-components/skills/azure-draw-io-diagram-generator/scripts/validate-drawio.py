#!/usr/bin/env python3
"""Validate draw.io structure, self-containment, and icon provenance."""
...
def _required_provenance_errors(
    ...
    cell_id = cell.get("id", UNKNOWN_CELL_ID)
    return [
...
def _provider_errors(cell: ET.Element, prefix: str) -> list[str]:
    ...
    if provider not in PROVIDERS:
        errors.append(
            f"{prefix} Official icon '{cell_id}' has unsupported "
            f"provider '{provider}'"
        )
...
def _source_errors(cell: ET.Element, prefix: str) -> list[str]:
    ...
    for field in ("iconSource", "iconTerms"):
        ...
        if not _https_url(value):
            errors.append(
                f"{prefix} Official icon '{cell_id}' {field} must "
                "be an HTTPS URL"
            )
        elif not _provider_owns_url(provider, value):
            errors.append(
                f"{prefix} Official icon '{cell_id}' {field} is not "
                "a recognized "
...
def _retrieval_date_errors(cell: ET.Element, prefix: str) -> list[str]:
    ...
    try:
        ...
        return [
            f"{prefix} Official icon '{cell_id}' iconRetrieved must "
            "use YYYY-MM-DD"
        ]
    if retrieved > date.today():
        return [
            f"{prefix} Official icon '{cell_id}' retrieval date is "
            "in the future"
        ]
    return []
...
def _label_and_style_errors(cell: ET.Element, prefix: str) -> list[str]:
    ...
    if "aspect=fixed" not in style or "imageAspect=1" not in style:
        errors.append(
            f"{prefix} Official icon '{cell_id}' must preserve its "
            "aspect ratio"
        )
    return errors
...
def _embedded_method_errors(cell: ET.Element, prefix: str) -> list[str]:
    ...
    if image is None:
        return [
            f"{prefix} Official icon '{cell_id}' has no embedded "
            "SVG image"
        ]
    svg_bytes = _embedded_svg_bytes(image)
    if svg_bytes is None:
        return [
            f"{prefix} Official icon '{cell_id}' has an invalid "
            "SVG data URI"
        ]
...
def _parent_errors(
    ...
    if parent not in ids:
        return [
            f"{prefix} Cell '{cell_id}' references unknown parent "
            f"'{parent}'"
        ]
    return []
...
def _endpoint_errors(
    ...
    if endpoint is not None and endpoint not in ids:
        return [
            f"{prefix} Edge '{cell_id}' has unknown {endpoint_name} "
            f"'{endpoint}'"
        ]
    return []
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Validate draw.io structure, self-containment, "
            "and official icon provenance."
        )
    )
