...
description: >-
  Deploys and operates Prometheus, Grafana, Loki, and Alertmanager for Open Horizons
  observability. Use this skill when deploying the monitoring stack, configuring dashboards,
  validating alert rules, checking metrics, reviewing SLO signals, or troubleshooting
  observability behavior.
---

<!-- Generated from harness/github-copilot/plugins/open-horizons-platform/skills/azure-observability-stack/SKILL.md by harness/claude-code/scripts/convert_from_copilot.py. Edit the source, not this file. -->
...
- Use `terraform-cli` (`skill`) instead when the task is Terraform IaC for observability infrastructure.
- Use `kubectl-cli` (`skill`) instead when the task is direct Kubernetes inspection outside observability workflows.
- Use `pipeline-diagnostics` (`skill`) instead when the task is CI/CD pipeline diagnostics.
...
| --- | --- | --- |
| `kubectl-cli` | `skill` | Kubernetes inspection or namespace operations are the primary task. |
| `helm-cli` | `skill` | Helm install, upgrade, rollback, or values operations are the primary task. |
| `terraform-cli` | `skill` | Terraform changes to the observability module are required. |
| `validation-scripts` | `skill` | Script-based post-deploy validation is required. |
| `pipeline-diagnostics` | `skill` | The problem is a GitHub Actions or CI/CD failure. |
