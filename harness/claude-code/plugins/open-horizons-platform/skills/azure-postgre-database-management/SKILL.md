...
<!-- Generated from harness/github-copilot/plugins/open-horizons-platform/skills/azure-postgre-database-management/SKILL.md by harness/claude-code/scripts/convert_from_copilot.py. Edit the source, not this file. -->
...
- Use `terraform-cli` (`skill`) instead when database infrastructure must be changed through Terraform.
- Use `deploy-orchestration` (`skill`) instead when database work is part of full platform deployment sequencing.
- Use `kubectl-cli` (`skill`) instead when the task is Kubernetes pod inspection.
...
| `terraform-cli` | `skill` | Database infrastructure changes must be planned or applied as IaC. |
| `kubectl-cli` | `skill` | Database troubleshooting requires Kubernetes workload inspection. |
| `observability-stack` | `skill` | Metrics, dashboards, or alerts should monitor database behavior. |
| `deploy-orchestration` | `skill` | Database checks are part of platform deployment. |
