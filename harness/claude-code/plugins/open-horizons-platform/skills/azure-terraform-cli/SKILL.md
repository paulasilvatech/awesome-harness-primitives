...
<!-- Generated from harness/github-copilot/plugins/open-horizons-platform/skills/azure-terraform-cli/SKILL.md by harness/claude-code/scripts/convert_from_copilot.py. Edit the source, not this file. -->
...
- Use `kubectl-cli` (`skill`) instead when the task is Kubernetes resource inspection, logs, events, manifests, rollout, or troubleshooting.
- Do not use this skill for Helm charts.
- Use `helm-cli` (`skill`) instead when the task is chart values, repositories, templates, releases, upgrades, or rollbacks.
- Do not use this skill for full deployment orchestration.
- Use `deploy-orchestration` (`skill`) instead when the task spans prerequisites, Terraform apply sequencing, Kubernetes verification, and platform validation.
...
| `validation-scripts` | `skill` | Running repository validation after Terraform changes. |

## Quality gate

- [ ] `name` is `terraform-cli` and matches the parent directory.
