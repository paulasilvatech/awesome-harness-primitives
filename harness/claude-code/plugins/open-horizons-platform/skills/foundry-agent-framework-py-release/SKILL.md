---
name: foundry-agent-framework-py-release
description: >-
  Use when cutting a Python release for the microsoft/agent-framework monorepo. Triggers on "bump
  py versions", "cut a python release", "prepare release PR for python", "release py packages",
  "bump python to X.Y.Z", or similar requests to bump Python package versions and prepare a
  release PR. Handles all four lifecycle tiers (alpha, beta, rc, released) with CHANGELOG-driven
  selective bumps, floor bound checks, and post-bump validation.
---

<!-- Generated from harness/github-copilot/plugins/open-horizons-platform/skills/foundry-agent-framework-py-release/SKILL.md by harness/claude-code/scripts/convert_from_copilot.py. Edit the source, not this file. -->

# Foundry Agent Framework Python Release

Cuts a Python release PR for the `microsoft/agent-framework` monorepo.

## When to invoke

- Preparing a Python package release for `microsoft/agent-framework`.
- Drafting the release CHANGELOG and deriving package version bumps from it.
- Validating lifecycle transitions, dependency floors, or release metadata.
...
- Release base: `<ref>`
- CHANGELOG section: `<release-label>`
- Packages bumped: `<package>: <old> -> <new>`
- Dependency floors updated: `<none or details>`
...
- [ ] The release base and live package tier map were refreshed.
- [ ] Every bumped package has a matching CHANGELOG entry.
- [ ] Coupled root/core versions and internal dependency floors are consistent.
- [ ] Release validation and relevant tests passed.
- [ ] The final report lists exact versions, checks, and the branch or PR.
