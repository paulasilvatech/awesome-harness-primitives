---
description: >-
  Prove behavior and data equivalence for one implemented COBOL/DB2 slice with requirement
  coverage, DB2 and VSAM mapping review, and recorded reconciliation numbers.
argument-hint: "scope=<implemented-slice> destination=response|<report-file>"
---

<!-- Generated from harness/github-copilot/plugins/mainframe-cobol-db2-classic/prompts/cobol-classic-verify.prompt.md by harness/claude-code/scripts/convert_from_copilot.py. Edit the source, not this file. -->

# /cobol-classic-verify
...
- Load the context, characterization, and DB2 migration Skills.
...
2. **Load context.** Load context, characterization, and DB2 migration Skills.
...
```text
/cobol-classic-verify scope=impl/001-payment-inspection destination=04-evolution/reports/001-payment-inspection.md
```
...
| --- | --- | --- |
| `cobol-classic-quality` | agent | Owns the quality-gate verdict and routing decisions. |
| `db2-postgresql-migration` | skill | Supplies the mapping rules and reconciliation procedure. |
| `legacy-characterization-testing` | skill | Supplies behavior-pinning oracles for the verified slice. |
