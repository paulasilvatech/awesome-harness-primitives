...
description: >-
  Guide Microsoft Entra ID app registration, OAuth 2.0 authentication, and MSAL integration. Use
  when the user wants to create or register an Entra ID application, configure OAuth, set up
  authentication, add API permissions, generate a service principal, review an MSAL or console app
  authentication example, or complete Entra ID tenant setup.
license: MIT
metadata:
  author: Microsoft
  version: 1.2.1
...
<!-- Generated from harness/github-copilot/plugins/azure-governance/skills/azure-entra-app-registration/SKILL.md by harness/claude-code/scripts/convert_from_copilot.py. Edit the source, not this file. -->

# Entra ID app registration

Register a Microsoft Entra ID application, choose the right OAuth 2.0 flow, assign least-privilege API permissions, and wire MSAL into the calling application without embedding long-lived secrets.

## When to invoke

- "Create an Entra ID app registration for my API."
- "Configure OAuth 2.0 and add API permissions."
- "Generate a service principal for this application."
- "Show an MSAL sign-in example for a console app."
...
**Status:** registered | configured | blocked
**Summary:** <one sentence covering scope and outcome>

### Details
Application and service principal identifiers, permissions, and auth flow.
...
- [ ] The selected OAuth flow matches the client type.
- [ ] Secrets are stored outside source control or replaced by certificates or federation.
