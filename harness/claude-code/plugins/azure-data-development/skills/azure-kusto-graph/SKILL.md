...
description: >-
  Build and query Kusto graphs from natural language, covering transient graphs with make-graph,
  persistent graph models and snapshots, pattern matching with graph-match, shortest paths,
  connected components, and graph-to-table export using edges-first construction. Use when the
  user works with make-graph, graph-match, graph-shortest-paths, graph-to-table, or
  graph-mark-components, builds a persistent graph model or snapshot, constructs a graph from
  data, finds paths between nodes, matches patterns in a graph, or analyzes connected components
  in KQL.
license: MIT
metadata:
  author: Microsoft
...
<!-- Generated from harness/github-copilot/plugins/azure-data-development/skills/azure-kusto-graph/SKILL.md by harness/claude-code/scripts/convert_from_copilot.py. Edit the source, not this file. -->
...
**Status:** query-ready | analyzed | blocked
**Summary:** <one sentence covering scope and outcome>

### Details
Edge and node definitions plus the graph query and its interpretation.
...
- [ ] Edges are defined before the graph is materialized.
- [ ] The transient versus persistent choice is justified.
