...
description: >-
  Apply IRQL graph functions to existing KQL or IRQL query results for Kusto Explorer
  visualization, generating Lift_To_Graph mappings and composing Graph_Render_View,
  Graph_Fold_By_Property, Extract_Node_*, Enrich_Node_*, and Enrich_Graph_* calls. Use when the
  user supplies a query and wants Lift_To_Graph, Graph_Render_View, Graph_Fold_By_Property, IRQL
  graph enrichment, graph mapping for existing results, an icon-decorated graph, or folded graph
  nodes.
license: MIT
metadata:
  author: Microsoft
...
<!-- Generated from harness/github-copilot/plugins/azure-data-development/skills/azure-kusto-irql-graph/SKILL.md by harness/claude-code/scripts/convert_from_copilot.py. Edit the source, not this file. -->
...
**Status:** mapping-ready | rendered | blocked
**Summary:** <one sentence covering scope and outcome>

### Details
Lift_To_Graph mapping and composed graph rendering calls.
...
- [ ] The mapping matches the supplied query result columns.
- [ ] Only graph composition was performed, not new query authoring.
