---
description: >-
  Harden, deliver, and document one verified SIFAP slice with evidence-based findings, safe action
  previews, and residual-risk ownership.
argument-hint: "scope=<verified-slice-or-pr> destination=response|<review-file>"
---

<!-- Generated from harness/github-copilot/plugins/mainframe-natural-adabas/prompts/sifap-operate.prompt.md by harness/claude-code/scripts/convert_from_copilot.py. Edit the source, not this file. -->
...
- Load SIFAP context, the loop skill, and task-relevant CI/CD, infrastructure, and security Skills.
...
2. **Load context.** Load SIFAP context, the loop skill, and task-relevant delivery, IaC, and security Skills.
...
```text
/sifap-operate scope=impl/001-payment-inspection destination=05-operations/reviews/001-payment-inspection.md
```
...
| --- | --- | --- |
| `sifap-operations` | agent | Owns readiness judgment and approval boundaries. |
| `sifap-loop` | skill | Evaluates the operations gate and records the iteration ledger. |
| `sifap-workshop-orchestration` | skill | Evaluates the stage exit gate and final handoff. |
