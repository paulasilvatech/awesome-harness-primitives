...
<!-- Generated from harness/github-copilot/plugins/mainframe-natural-adabas/agents/sifap-operations.agent.md by harness/claude-code/scripts/convert_from_copilot.py. Edit the source, not this file. -->
...
Before acting, load `sifap-modernization-context`, `sifap-loop`, and `sifap-workshop-orchestration`, plus
...
- [ ] Required context, loop, orchestration, and operational Skills were loaded.
...
| `sifap-quality` | agent | A behavior, test, or data-equivalence finding appears during delivery | Finding, evidence, affected REQ-ID, and the check that surfaced it. |
| `sifap-builder` | agent | A code finding requires repair | Finding, evidence, affected REQ-ID, expected behavior, and validation command. |
| `sifap-architect` | agent | A requirement, topology, or intentional drift decision is needed | Decision, alternatives, constraints, and migration impact. |
