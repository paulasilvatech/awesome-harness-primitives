...
| `extraction` | extractor output | Counts recognized Natural/JCL members and DDM files and records unresolved references. |
...
`scripts/sifap_loop_extract.py` builds the legacy layer from the corpus. The boundary is deliberate:

| Extracted by the tool | Authored by a person |
| --- | --- |
| Members, from the file extension and name | `BusinessRule`, `Requirement`, `Decision` |
| `CALLNAT`, `PERFORM`, `INCLUDE`, `USES_MAP`, `USES_DATA_AREA`, `RUNS` | `DERIVES_RULE`, `SATISFIED_BY`, `DECIDED_BY` |
| `READS`, `STORES`, resolved through a `VIEW OF` declaration | `JavaType`, `Endpoint`, `PgTable`, `Test`, `Workflow`, `Runbook` |
| `AdabasFile` and `Ddm` presence, and `DEFINED_BY` | `AdabasField`, `Descriptor`, `HAS_FIELD`, `INDEXED_BY`, `MIGRATES_TO` |
| Node and edge `evidence` with a line anchor | `status`, `owner`, `note`, `greenfield` |

Field-level DDM content is not extracted: DDM report layouts vary too much between environments to parse
without inventing structure. Use `natural-adabas-analysis` to read fields and descriptors, and
`adabas-postgresql-migration` to map them.

What the tool cannot cite becomes an `unresolved` note on stderr rather than a node: a `CALLNAT` target
absent from the corpus, an access operand with no `VIEW OF` declaration, a bare `UPDATE` or `DELETE`, and
a member name used by more than one file. Comments are stripped before matching, so a commented-out
`CALLNAT` produces no edge. Run with `--strict` to fail on any unresolved reference.
