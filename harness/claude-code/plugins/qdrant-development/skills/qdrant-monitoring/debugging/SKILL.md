...
- Use `/collections/{collection_name}/optimizations` endpoint (v1.17+) to check status [Optimization monitoring](https://qdrant.tech/documentation/operations/optimizer/?s=optimization-monitoring)
...
- Web UI has an Optimizations tab with timeline view and per-task duration metrics [Web UI](https://qdrant.tech/documentation/operations/optimizer/?s=web-ui)
...
- Estimate expected memory: `num_vectors * dimensions * 4 bytes * 1.5` for vectors, plus payload and index overhead [Capacity planning](https://qdrant.tech/documentation/operations/capacity-planning/)
...
- Compare filtered vs unfiltered query times. Large gap means missing payload index. [Payload index](https://qdrant.tech/documentation/manage-data/indexing/?s=payload-index)
