...
name: msstore-cli
description: >-
  Use Microsoft Store Developer CLI (msstore) to configure Partner Center credentials, list Store
  apps, package and publish Windows submissions, check status, manage package flights, gradual
  rollouts, and CI/CD publishing for Windows App SDK/WinUI, UWP, .NET MAUI, Flutter, Electron,
  React Native, and PWA apps. Use when asked to configure Store credentials, publish to Microsoft
  Store, check submission status, or automate Store releases.
license: MIT
---

<!-- Generated from harness/github-copilot/plugins/dotnet-desktop-development/skills/msstore-cli/SKILL.md by harness/claude-code/scripts/convert_from_copilot.py. Edit the source, not this file. -->
