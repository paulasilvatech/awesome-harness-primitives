...
- [the `mlv` mode](../../mlv.md) — schedule, trigger, monitor, and cancel MLV refreshes via REST API (use when user asks to automate refresh, not author SQL)
...
```
Keep Livy resource fields such as `driverMemory` and `executorMemory` at the
top level. Use `conf` only for Spark properties.
```text
