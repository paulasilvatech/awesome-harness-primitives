...
See [deploy-checklist-template.md § Deployment Summary](deploy/deploy-checklist-template.md) for the full deployment summary format.

## Artifact Self-Check

> ⛔ **Artifact self-check — MANDATORY before handoff.** Verify these exist before presenting cleanup or next steps:
> 1. `deploy-result.json` in session folder — if missing, read [`deploy-schemas.ts`](deploy/deploy-schemas.ts) and write it NOW with status, endpoints, health, `orphanedResourceGroups[]`
...
| Auth/OAuth/MSAL detected in intent or prereq | **`azure-entra-app-registration`** — "Set up app registration for your auth flow" |
