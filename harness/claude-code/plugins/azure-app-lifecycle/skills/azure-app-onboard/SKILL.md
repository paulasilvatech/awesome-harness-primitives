...
description: >-
  Orchestrate an app end to end from a business idea, app idea, or existing codebase to a running
  Azure deployment with cost estimates and a pre-deploy approval gate, auto-detecting Azure
  services, scaffolding infrastructure code, and deploying without a generic template. Use when
  the user wants to bring an app to Azure, plan an Azure deployment, learn what Azure services an
  app needs, check the cost to run it, deploy an app or all its services to the cloud, migrate an
  existing app to Azure, or start a new app from no code.
license: MIT
metadata:
  author: Microsoft
  version: 1.2.3
...
<!-- Generated from harness/github-copilot/plugins/azure-app-lifecycle/skills/azure-app-onboard/SKILL.md by harness/claude-code/scripts/convert_from_copilot.py. Edit the source, not this file. -->
...
| 5 | **Plan architecture** | Write `prepare-plan.json`. | ⛔ **You MUST read [references/phases/prepare.md](references/phases/prepare.md)** |
...
| 7 | **Scaffold** | Generate IaC, self-review. Write `scaffold-manifest.json`. Update `context.json`. | ⛔ **You MUST read [references/phases/scaffold.md](references/phases/scaffold.md)** |
...
| 9 | **Deploy** | Execute IaC, health-check. Write `deploy-result.json`. | ⛔ **You MUST read [references/phases/deploy.md](references/phases/deploy.md)** |
...
**Status:** planned | scaffolded | deployed | blocked
**Summary:** <one sentence covering scope and outcome>

### Details
Phase reached, Azure services selected, cost estimate, and approval decisions.
...
- [ ] Each phase artifact exists before the next phase started.
- [ ] The deploy approval gate was presented and answered before any deployment.
