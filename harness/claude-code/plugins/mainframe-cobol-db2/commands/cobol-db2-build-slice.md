---
description: >-
  Implement one approved COBOL/DB2 slice with behavior-pinning tests, recorded decisions,
  traceable validation, and explicit drift classification.
argument-hint: "slice=<NNN-slug> requirements=REQ-NNN[,REQ-NNN]"
---

<!-- Generated from harness/github-copilot/plugins/mainframe-cobol-db2/prompts/cobol-db2-build-slice.prompt.md by harness/claude-code/scripts/convert_from_copilot.py. Edit the source, not this file. -->

# /cobol-db2-build-slice
...
- The `cobol-db2-context`, `cobol-db2-loop`, and `legacy-characterization-testing` skills are available.
...
```text
/cobol-db2-build-slice slice=001-payment-inspection requirements=REQ-021,REQ-022
```
...
| --- | --- | --- |
| `cobol-db2-builder` | agent | Owns implementation judgment and drift classification. |
| `cobol-db2-loop` | skill | Evaluates the implementation gate and records the ledger. |
| `cobol-db2-verify` | prompt | Proves behavior and data equivalence for the slice. |
