...
description: >-
  Compare SQL Server tables across source and target instances for migration validation, ETL
  verification, production versus staging checks, schema drift, missing rows, extra rows, and
  column mismatches. Use this skill when the user asks for SQL Server reconciliation reports, and
  use mssql-python with Apache Arrow for fast extraction.
---

<!-- Generated from harness/github-copilot/plugins/data-engineering/skills/sql-server-table-reconciliation/SKILL.md by harness/claude-code/scripts/convert_from_copilot.py. Edit the source, not this file. -->
