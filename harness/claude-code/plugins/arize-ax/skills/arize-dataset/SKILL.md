...
name: arize-dataset
description: >-
  Creates, manages, and queries Arize datasets and examples. Covers dataset CRUD, appending
  examples, exporting data, and file-based dataset creation using the ax CLI. Use when the user
  needs test data, evaluation examples, or mentions create dataset, list datasets, export dataset,
  append examples, dataset version, golden dataset, or test set.
metadata:
  author: arize
...
<!-- Generated from harness/github-copilot/plugins/arize-ax/skills/arize-dataset/SKILL.md by harness/claude-code/scripts/convert_from_copilot.py. Edit the source, not this file. -->

# Arize dataset

Create, manage, query, append, export, and delete Arize datasets with the `ax` CLI, preserving space, version, schema, and credential-handling rules.

## When to invoke

- "Create an Arize dataset from this file."
- "List datasets in my Arize space."
- "Export this golden dataset for offline analysis."
- "Append examples to an evaluation dataset."
- "Troubleshoot an ax dataset command."
...
```markdown
## Arize dataset result

**Status:** complete | needs credentials | blocked
**Space:** `SPACE` or `ARIZE_SPACE`
**Dataset:** `DATASET_NAME` / `NAME_OR_ID`
**Version:** `VERSION_ID` or latest

| Action | Command | Evidence | Notes |
| --- | --- | --- | --- |
| list|get|create|append|export|delete | `ax datasets ...` | `<stdout, file path, or ID>` | `<schema, count, or blocker>` |

**Files**
- `<dataset_abc123_20260305_141500/examples.json or input file>`

**Next step**
- `<run experiment, re-export with --all, fix profile, or no action>`
```

## Quality gate

- [ ] `name` is `arize-dataset` and matches the parent directory.
- [ ] Every command uses the correct `--space SPACE`, `DATASET_NAME`, `NAME_OR_ID`, `VERSION_ID`, and profile context.
- [ ] `id`, `created_at`, and `updated_at` are never included in create/append payloads.
- [ ] Append input uses exactly one of `--json` or `--file`, and examples are non-empty JSON objects.
- [ ] Export count is checked; exactly 500 rows triggers a re-run with `--all`.
- [ ] Credential problems are handled through `ax profiles` or `ax ai-integrations`; `.env` files are not read.
- [ ] Referenced bundled resources exist: `references/ax-setup.md` and `references/ax-profiles.md`.
