...
Before acting, load `sifap-modernization-context`, `sifap-workshop-orchestration`, and the task-relevant
...
| `sifap-builder` | agent | A code or test finding requires repair | Finding, evidence, affected REQ-ID, expected behavior, and validation command. |
