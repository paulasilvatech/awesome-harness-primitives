---
name: 'cobol-db2-specify'
description: 'Promote approved COBOL/DB2 rule candidates into REQ-NNN requirements with source lineage, decision records, and a dependency-ordered slice plan.'
argument-hint: 'candidates=<rule-card-path> destination=specs/<NNN>-<feature>'
agent: 'cobol-db2-architect'
---
...
- The `cobol-db2-context`, `cobol-db2-loop`, and `sdd-requirements-engineer` skills are available.
...
```text
/cobol-db2-specify candidates=01-archaeology/rules/payment-duplicates.md destination=specs/001-payment-inspection
```
...
| --- | --- | --- |
| `cobol-db2-architect` | agent | Owns requirement and design judgment. |
| `cobol-db2-loop` | skill | Evaluates the architecture gate and records the ledger. |
| `cobol-db2-build-slice` | prompt | Implements the approved slice. |
