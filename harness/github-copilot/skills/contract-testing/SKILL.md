...
description: >-
  Design consumer-driven contract tests with Pact or provider schema
  verification so services can deploy independently without integration-wide
  end-to-end suites. Use when the user asks about contract testing, Pact,
  consumer-driven contracts, breaking API changes between services, provider
  verification, a contract broker, or can-i-deploy checks in CI.
license: MIT
---
