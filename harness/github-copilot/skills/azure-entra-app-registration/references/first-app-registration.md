...
4. Choose expiration:
   ...
   - **For production:** do not create a client secret; use workload identity federation,
     managed identity, or a certificate-backed credential with an approved rotation policy
5. Click **"Add"**
...
```bash
# Supply the development-only value through a secure environment or secret store.
...
- Prefer workload identity federation or managed identity for production
- Use Azure Key Vault only when a production credential cannot be eliminated
