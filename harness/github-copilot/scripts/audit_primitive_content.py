...
def shared_packaging() -> tuple[
    ...
    for plugin_dir in plugin_manifest_dirs():
        ...
        for ref in config.get("sharedSkills", []):
...
def plugin_owned_components(
    plugin_dir: Path,
    data: dict[str, Any],
) -> dict[str, list[Path]]:
    shared_skill_names = {
        Path(ref.rstrip("/")).name
        for ref in repository_config(data).get("sharedSkills", [])
        if isinstance(ref, str)
    }
    return {
...
def strict_validation() -> dict[str, Any]:
    validator = Validator(HARNESS_ROOT, quiet=True)
...
def library_content_units(
    ...
    for path in sorted((HARNESS_ROOT / "hooks").glob("*/hooks.json")):
        units.append(
            make_unit(
                kind="hook",
        ...
        units.append(
            make_unit(
                ...
                ownership=(
                    "shared"
                    if config.get("componentSource") == "library"
                    else str(config.get("componentSource", "unknown"))
                ),
                package=plugin_dir.name,
...
def render_report(audit: dict[str, Any]) -> str:
    ...
    shared_validation = audit["validation"]["sharedHarness"]
...
def has_strict_issues(audit: dict[str, Any]) -> bool:
    shared = audit["validation"]["sharedHarness"]
...
def report_stale_outputs(stale: list[str]) -> None:
    print(
        ...
        "python3 harness/github-copilot/scripts/audit_primitive_content.py",
        file=sys.stderr,
    )
    for path in stale:
