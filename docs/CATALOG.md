...
| ai-team-orchestration | 2.0.3 | active | static-validated | repository | Run a lightweight, role-separated AI development team with flexible tools, developer-selected models, proportionate planning, and optional QA. |
| apng-studio | 1.0.4 | active | runtime-required | upstream-mirror | Interactive GitHub Copilot app canvas extension for building Animated PNG (APNG) files from frames. Draw or upload frames, tune per-frame timing and compositing, preview live, sen… |
| application-security | 1.0.0 | active | static-validated | repository | Assess and harden application security: source vulnerability review, secret scanning, CodeQL scanning, STRIDE and TM7 threat models, data-breach blast radius, and GDPR engineering… |
| arcade-canvas | 1.0.4 | active | runtime-required | upstream-mirror | Play five retro Phaser mini-games in a Copilot canvas while agents work. |
| arch | 1.0.2 | active | static-validated | repository | Architecture and modernization toolkit: produce a cited architecture document for a locally-cloned repo, and generate a phased modernization plan that auto-runs Documentation mode… |
| arize-ax | 1.0.2 | active | static-validated | repository | Arize AX platform skills for LLM observability, evaluation, and optimization. Includes trace export, instrumentation, datasets, experiments, evaluators, AI provider integrations,… |
...
| azure-developer-tooling | 1.0.0 | active | static-validated | repository | Use Azure developer tooling day to day: Azure Developer CLI projects, Container Registry and DevOps CLI operations, Static Web Apps, Bicep deployment preflight, Azure Verified Mod… |
...
| career-productivity | 1.0.0 | active | static-validated | repository | Support individual work output: impact and brag sheets, performance and peer reviews, job-search materials, daily preparation, professional emails, meeting minutes, LinkedIn posts… |
| cast-imaging | 1.0.3 | active | static-validated | repository | A comprehensive collection of specialized agents for software analysis, impact assessment, structural quality advisories, and architectural review using CAST Imaging. |
...
| diagram-authoring | 1.0.0 | active | static-validated | repository | Author technical diagrams as source files: draw.io mxGraph XML, Excalidraw JSON, PlantUML ASCII output, and D2 entity-relationship diagrams generated from Entity Framework Core mo… |
| diagram-viewer | 1.0.4 | active | runtime-required | upstream-mirror | Render diagrams, click nodes to drill down, and view agent-generated explanations directly in the canvas. |
| dotnet-desktop-development | 1.0.0 | active | static-validated | repository | Build Windows and cross-platform .NET client applications with the MVVM Toolkit, dependency injection, messaging, Windows metadata APIs, WinUI 3 migration, Microsoft Store publish… |
| doublecheck | 1.0.2 | active | static-validated | repository | Three-layer verification pipeline for AI output. Extracts claims, finds sources, and flags hallucination risks so humans can verify before acting. |
| edge-ai-tasks | 1.0.2 | active | static-validated | repository | Task Researcher and Task Planner for intermediate to expert users and large codebases - Brought to you by microsoft/edge-ai |
| ember | 1.2.4 | active | runtime-required | upstream-mirror | An AI partner, not a tool. Ember carries fire from person to person — helping humans discover that AI partnership isn't something you learn, it's something you find. |
| eyeball | 1.0.2 | active | static-validated | repository | Document analysis with inline source screenshots. When you ask Copilot to analyze a document, Eyeball generates a Word doc where every factual claim includes a highlighted screens… |
| fabric-agentic-plugin | 1.0.2 | active | runtime-required | repository | Microsoft Fabric agentic operations toolkit with specialist agents, progressive workload guides, migration workflows, Power BI resources, and MCP integrations for Fabric IQ and SQ… |
| fastah-ip-geo-tools | 0.0.11 | incubating | static-validated | repository | This plugin is for network operations engineers who wish to tune and publish IP geolocation feeds in RFC 8805 format. It consists of an AI Skill and an associated MCP server that… |
...
| git-github-workflow | 1.0.0 | active | static-validated | repository | Run day-to-day Git and GitHub workflows: conventional commits and branches, issue and release management, pull request review loops, dashboards, Dependabot, and contribution guard… |
...
| openapi-to-application-csharp-dotnet | 1.0.2 | active | static-validated | repository | Generate production-ready .NET applications from OpenAPI specifications. Includes ASP.NET Core project scaffolding, controller generation, entity framework integration, and C# bes… |
| openapi-to-application-go | 1.0.2 | active | static-validated | repository | Generate production-ready Go applications from OpenAPI specifications. Includes project scaffolding, handler generation, middleware setup, and Go best practices for REST APIs. |
| openapi-to-application-java-spring-boot | 1.0.2 | active | static-validated | repository | Generate production-ready Spring Boot applications from OpenAPI specifications. Includes project scaffolding, REST controller generation, service layer organization, and Spring Bo… |
| openapi-to-application-nodejs-nestjs | 1.0.2 | active | static-validated | repository | Generate production-ready NestJS applications from OpenAPI specifications. Includes project scaffolding, controller and service generation, TypeScript best practices, and enterpri… |
| openapi-to-application-python-fastapi | 1.0.2 | active | static-validated | repository | Generate production-ready FastAPI applications from OpenAPI specifications. Includes project scaffolding, route generation, dependency injection, and Python best practices for asy… |
| oracle-to-postgres-migration-expert | 1.1.2 | active | static-validated | repository | Expert agent for Oracle-to-PostgreSQL application migrations in .NET solutions. Performs code edits, runs commands, and invokes extension tools to migrate .NET/Oracle data access… |
| ospo-sponsorship | 1.0.2 | active | static-validated | repository | Tools and resources for Open Source Program Offices (OSPOs) to identify, evaluate, and manage sponsorship of open source dependencies through GitHub Sponsors, Open Collective, and… |
| partners | 1.0.3 | active | static-validated | repository | Custom agents that have been created by GitHub partners |
...
| power-platform-mcp-connector-development | 1.0.2 | active | static-validated | repository | Complete toolkit for developing Power Platform custom connectors with Model Context Protocol integration for Microsoft Copilot Studio |
| pr-artifact-explorer | 1.0.2 | active | runtime-required | upstream-mirror | Navigate pull requests and securely explore GitHub Actions artifacts, including test results, static sites, terminal recordings, and source files. |
| project-documenter | 1.0.2 | active | static-validated | repository | Generate professional project documentation with draw.io architecture diagrams and Word (.docx) output with embedded images. Automatically discovers any project's technology stack… |
| project-planning | 1.0.2 | active | static-validated | repository | Tools and guidance for software project planning, feature breakdown, epic management, implementation planning, and task organization for development teams. |
| python-mcp-development | 1.0.2 | active | static-validated | repository | Complete toolkit for building Model Context Protocol (MCP) servers in Python using the official SDK with FastMCP. Includes instructions for best practices, a prompt for generating… |
...
| roundup | 1.0.2 | active | static-validated | repository | Self-configuring status briefing generator. Learns your communication style from examples, discovers your data sources, and produces draft updates for any audience on demand. |
| ruby-mcp-development | 1.0.2 | active | static-validated | repository | Complete toolkit for building Model Context Protocol servers in Ruby using the official MCP Ruby SDK gem with Rails integration support. |
| rug-agentic-workflow | 1.0.2 | active | static-validated | repository | Three-agent workflow for orchestrated software delivery with an orchestrator plus implementation and QA subagents. |
| rust-mcp-development | 1.0.3 | active | static-validated | repository | Build high-performance Model Context Protocol servers in Rust using the official rmcp SDK with async/await, procedural macros, and type-safe implementations. |
| salesforce-development | 1.1.2 | active | static-validated | repository | Complete Salesforce agentic development environment covering Apex & Triggers, Flow automation, Lightning Web Components, Aura components, and Visualforce pages. |
| security-best-practices | 1.0.2 | active | static-validated | repository | Security frameworks, accessibility guidelines, performance optimization, and code quality best practices for building secure, maintainable, and high-performance applications. |
| signals-dashboard | 0.2.2 | incubating | runtime-required | upstream-mirror | Real-time Workshop dashboard with agent signals, honesty calibration, and cost-aware repo or connected desk launch profiles. |
...
| typescript-mcp-development | 1.0.2 | active | static-validated | repository | Complete toolkit for building Model Context Protocol (MCP) servers in TypeScript/Node.js using the official SDK. Includes instructions for best practices, a prompt for generating… |
| typespec-m365-copilot | 1.0.2 | active | static-validated | repository | Comprehensive collection of prompts, instructions, and resources for building declarative agents and API plugins using TypeSpec for Microsoft 365 Copilot extensibility. |
