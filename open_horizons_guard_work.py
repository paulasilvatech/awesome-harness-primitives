...
RISK_PATTERNS = (
    (
        re.compile(
            r"\bterraform(?:[\s/_.:-]+)(?:apply|destroy)\b", re.IGNORECASE),
    ...
    (
        re.compile(
            r"\bkubectl(?:[\s/_.:-]+)(?:delete|drain)\b", re.IGNORECASE),
    ...
    (
        re.compile(
            r"\bargocd(?:[\s/_.:-]+)app(?:[\s/_.:-]+)delete\b", re.IGNORECASE),
        "Argo CD application deletion",
    ),
    (
        re.compile(
            r"\bgh(?:[\s/_.:-]+)repo(?:[\s/_.:-]+)delete\b", re.IGNORECASE),
    ...
    (
        re.compile(
            r"\b(?:drop\s+(?:database|table)|truncate\s+table)\b", re.IGNORECASE),
...
def additional_decision(
    ...
    if is_edit_tool(name):
        if APP_CONFIG_PATH.search(combined) and SECRET_LITERAL.search(combined):
            return (
                "deny",
        ...
        if any(pattern.search(combined) for pattern in PROTECTED_PATHS):
            return (
                "ask",
