...
5. Generate the subject from the diff: type, optional scope, and imperative description under 72 characters.
6. Execute `git commit -m "<type>[scope]: <description>"` when the user asked to commit. For a body or footer, pass separate `-m` arguments rather than embedding command substitutions.
7. If hooks fail, fix the hook findings and create a new commit attempt. Do not amend unless the user explicitly asks.
