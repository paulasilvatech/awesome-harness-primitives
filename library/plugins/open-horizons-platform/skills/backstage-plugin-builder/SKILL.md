...
> [!NOTE]
> This skill depends on official Backstage documentation, preferably through `mcp-ecosystem` documentation tools. It may shell out to bundled Python scripts under `scripts/` and to package commands in the target Backstage workspace. Resolve bundled paths relative to this `SKILL.md`.
...
- Official docs are checked through `mcp-ecosystem` or fallback references in `references/`.
...
- Read `references/mcp-doc-validation.md`.
- Read `references/official-docs.md`.
- Load the type-specific reference, such as `references/frontend-plugin.md` or `references/backend-module.md`.
- If MCP documentation lookup fails, run:

```bash
python scripts/validate_official_docs.py
```
...
```bash
python scripts/create_backstage_plugin_artifacts.py \
  --plugin-id my-plugin \
...
```bash
python scripts/validate_backstage_plugin.py <plugin-dir>
python scripts/validate_backstage_plugin.py <plugin-dir> --run
```
...
```bash
python scripts/generate_quality_hooks.py --root .
```
