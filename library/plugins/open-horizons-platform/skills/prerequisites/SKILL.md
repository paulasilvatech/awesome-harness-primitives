...
Use this skill to validate the operator workstation or CI runner before Open Horizons deployment. It produces a tool and authentication report using repository `scripts/validate-prerequisites.sh` when present or the bundled `scripts/validate-prerequisites.sh` and `scripts/validate-cli-prerequisites.sh`. Resolve bundled paths relative to this `SKILL.md`.
