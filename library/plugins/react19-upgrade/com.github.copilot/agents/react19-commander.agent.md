...
description: >-
  Orchestrates complete React 19 migrations through audit, dependency, source, and test specialists with strict gates and memory state. Use to coordinate a zero-incomplete React 18 to React 19 upgrade pipeline.
...
Required test fixes include `act` import from `react-dom/test-utils` → `react`, `Simulate` → `fireEvent` from `@testing-library/react`, StrictMode spy call count deltas, `useRef(null)` shape updates, and custom render helper verification.
