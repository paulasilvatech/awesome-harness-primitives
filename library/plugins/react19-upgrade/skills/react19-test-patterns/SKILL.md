...
| `Simulate` | `fireEvent` from `@testing-library/react` | Keep event payloads such as `{ target: { value: 'hello' } }`. |
| `renderIntoDocument` | `render` from `@testing-library/react` | Prefer queries from the returned render result. |
...
- **Async warnings often need awaited user actions**: after upgrading `@testing-library/user-event`, many calls require `await` before assertions.
