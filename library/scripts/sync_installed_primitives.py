#!/usr/bin/env python3
"""Synchronize declared repository customizations from canonical library sources."""
...
def find_repository_root(start: Path) -> Path:
    for candidate in (start.resolve(), *start.resolve().parents):
        has_checkout = (candidate / ".git").exists() and (candidate / "library").is_dir()
        has_sentinels = (
            (candidate / "library" / "installed-primitives.json").is_file()
            and (candidate / ".github").is_dir()
            and (candidate / "docs").is_dir()
        )
        if has_checkout or has_sentinels:
            return candidate
    raise ValueError(f"repository root not found from {start}")


REPO_ROOT = find_repository_root(Path(__file__).resolve())
DEFAULT_MANIFEST = REPO_ROOT / "library" / "installed-primitives.json"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="copy manifest (default: library/installed-primitives.json)",
    )
    parser.add_argument(
        "--check",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    repo_root = find_repository_root(manifest)
    try:
