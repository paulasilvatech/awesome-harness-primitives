...
- **Verify versions with commands.** Confirm `react` and `react-dom` through Node, and confirm `@testing-library/react` through `npm ls`.
...
3. **Upgrade Testing Library.** Install `@testing-library/react@^16.0.0`, `@testing-library/jest-dom@^6.0.0`, and `@testing-library/user-event@^14.0.0` because RTL 14 and below uses `ReactDOM.render` internally.
...
| @testing-library/react | `16.x` | <version> | pass/fail |
...
- [ ] `@testing-library/react@^16.0.0`, `@testing-library/jest-dom@^6.0.0`, and `@testing-library/user-event@^14.0.0` were installed or a failure was reported.
