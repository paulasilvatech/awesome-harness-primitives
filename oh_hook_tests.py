...
class OpenHorizonsSafetyTests(unittest.TestCase):
    ...
    def test_windows_literal_secret_is_denied(self) -> None:
        result = GUARD.additional_decision(
            "preToolUse",
            "edit",
            (
                "C:\\repo\\backstage\\app-config.local.yaml "
    ...
    def test_windows_protected_path_requires_approval(self) -> None:
        result = GUARD.additional_decision(
            "preToolUse",
            "write",
            "C:\\repo\\packages\\backend\\src\\plugins\\auth.ts",
        )
        self.assertEqual(result[0], "ask")
