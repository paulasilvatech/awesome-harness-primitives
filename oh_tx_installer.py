...
def rollback_install(
    ...
    if state_touched:
        try:
            ...
            if state_backup is not None and state_backup.exists():
                os.replace(state_backup, state_destination)
    ...
    for destination, backup in reversed(writes):
        try:
            ...
            errors.append(f"file rollback failed for {destination}: {exc}")
    for archive, destination in reversed(archives):
        try:
            ...
            errors.append(f"archive rollback failed for {destination}: {exc}")
    return errors
...
def apply_install(
    ...
    for entry in plan:
        ...
        elif entry.status in {"create", "update"}:
    ...
    with tempfile.TemporaryDirectory(
        ...
        for index, entry in enumerate(write_entries):
            ...
            if digest_bytes(content) != entry.sha256:
                raise ValueError(
                    f"source changed while applying: {entry.source}"
                )
